...
11. **Dokumentationsinkonsistenz in `AGENTS.md` – offen.** Dort steht noch direkter Browserstart als aktueller Runtime-Ansatz, obwohl README und Startskript den lokalen Server empfehlen.
12. **Beschädigtes Qualitäts-Gate in `AGENTS.md` – offen.** Tippfehler, grammatisch unklare Pflichtpunkte und ein leerer Punkt 12 schwächen die Verbindlichkeit. Separat korrigieren, damit diese Robustheitsänderung klein und rückbaubar bleibt.
13. **Entwicklerdokumentation veraltet – offen.** Sie nennt Tastaturplatzierung weiterhin als nächsten Schritt, obwohl diese laut TODO bereits umgesetzt ist.
...
3. CI-Workflow für alle vorhandenen Prüfungen.
4. Dokumentationsbereinigung in `AGENTS.md` und `docs/DEVELOPER_GUIDE.md`.
