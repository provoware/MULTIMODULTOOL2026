...
verify_package() {
    ...
    [[ -r "$sidecar" ]] || fail "SHA-256-Sidecar fehlt: $sidecar" 39
    (cd "$(dirname "$package")" && sha256sum --quiet -c "$(basename "$sidecar")") \
        || fail "Paket-SHA-256 ist ungültig." 40
    local metadata
...
prepare_state() {
    ...
    [[ ! -L "$STATE_ROOT" && ! -L "$ARCHIVE_ROOT" ]] || fail "Releasezustand darf kein Symlink sein." 41
}
...
install_package() {
    ...
    command -v apt-get >/dev/null 2>&1 || fail "apt-get fehlt." 42
    command -v python3 >/dev/null 2>&1 || fail "Python 3 fehlt." 43
    python3 -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' \
        || fail "Python 3.10 oder neuer wird benötigt." 44
    prepare_state
...
rollback_package() {
    ...
    confirm_mutation
    prepare_state
    read_env_file "$PREVIOUS_FILE" || fail "Kein geprüfter vorheriger Paketstand für Rollback vorhanden." 45
    local rollback_version="$VERSION" rollback_build="$BUILD_ID" rollback_archive="$ARCHIVE"
    [[ -r "$rollback_archive" ]] || fail "Archiviertes Rollback-Paket fehlt." 46
...
purge_user_data() {
    ...
    [[ -n "$user" && "$user" != "root" ]] || fail "Für den Nutzerdaten-Purge muss MMT_PURGE_USER oder SUDO_USER auf einen Nicht-root-Nutzer zeigen." 47
    local entry home uid
    entry="$(getent passwd "$user")" || fail "Nutzer für Purge nicht gefunden: $user" 48
    IFS=: read -r _ _ uid _ _ home _ <<<"$entry"
    [[ -d "$home" && "$home" == /home/* ]] || fail "Unsicheres Nutzer-Home für Purge: $home" 49
    local paths=(
    ...
    for path in "${paths[@]}"; do
        [[ ! -L "$path" ]] || fail "Purge-Ziel ist ein Symlink und wurde blockiert: $path" 50
        [[ "$path" == "$home"/* || "$path" == "/run/user/$uid/multimodultool2026" ]] \
            || fail "Purge-Ziel verlässt die erlaubte Grenze: $path" 51
...
uninstall_package() {
    ...
    dpkg-query -W -f='${Status}' "$PACKAGE_NAME" 2>/dev/null | grep -q 'install ok installed' \
        && fail "Paket ist nach Entfernung noch installiert." 52
    [[ ! -e /usr/bin/multimodultool2026 ]] || fail "Starter blieb nach Entfernung zurück." 53
    [[ ! -e /usr/lib/multimodultool2026 ]] || fail "Runtime blieb nach Entfernung zurück." 54
    [[ ! -e /usr/share/applications/multimodultool2026.desktop ]] || fail "Desktopdatei blieb nach Entfernung zurück." 55
    [[ "$PURGE_USER_DATA" -eq 0 ]] || purge_user_data
    if [[ "$PURGE_SYSTEM_STATE" -eq 1 ]]; then
        [[ ! -L "$STATE_ROOT" ]] || fail "Systemzustand ist ein Symlink und wurde nicht gelöscht." 56
